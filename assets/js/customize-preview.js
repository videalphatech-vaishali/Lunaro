...
( function ( $, api, _ ) {
	...
	api.selectiveRefresh.partialConstructor.cover_fixed =
		api.selectiveRefresh.Partial.extend( {
			/**
	...
	api.selectiveRefresh.partialConstructor.cover_opacity =
		api.selectiveRefresh.Partial.extend( {
			/**
			 * Input attributes.
			 *
			...
			refresh: function () {
				...
				if ( cover.length ) {
					...
					className =
						classNames[
							ranges.indexOf( parseInt( setting.get(), 10 ) )
						];
	...
	api( 'header_footer_background_color', function ( value ) {
		value.bind( function ( to ) {
			...
			$( 'body:not(.overlay-header)#site-header, #site-footer' ).css(
				'background-color',
				to
			);
			...
			if (
				to.toLowerCase() ===
				api( 'background_color' ).get().toLowerCase()
			) {
	...
	api( 'background_color', function ( value ) {
		value.bind( function ( to ) {
			...
			if (
				to.toLowerCase() ===
				api( 'header_footer_background_color' ).get().toLowerCase()
			) {
				$( 'body' ).addClass( 'reduced-spacing' );
			} else {
				$( 'body' ).removeClass( 'reduced-spacing' );
	...
	api( 'accent_hue', function ( value ) {
		value.bind( function () {
			...
			setTimeout( function () {
				Object.keys( twentyTwentyBgColors ).forEach(
					function ( context ) {
	...
	Object.keys( twentyTwentyBgColors ).forEach( function ( context ) {
		wp.customize(
			twentyTwentyBgColors[ context ].setting,
			function ( value ) {
				value.bind( function () {
					// Generate the styles.
					// Add a small delay to be sure the accessible colors were generated.
	...
	function twentyTwentyGenerateColorA11yPreviewStyles( context ) {
		...
		var a11yColors = window.parent.wp
		...
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			...
			if ( twentyTwentyPreviewEls[ context ] ) {
				_.each(
					twentyTwentyPreviewEls[ context ],
