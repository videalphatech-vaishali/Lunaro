			...
			modal.addEventListener(
				...
				function ( event ) {
					var styles = htmlStyles(),
						...
						paddingTop =
							Math.abs( getAdminBarHeight() ) - offsetY + 'px',
			...
			modal.addEventListener(
				...
				function ( event ) {
					...
					setTimeout( function () {
						...
						Object.keys( htmlStyles() ).forEach(
							function ( styleKey ) {
						...
						_win.scrollTo(
							0,
							Math.abs(
								_win.twentytwenty.scrolled + getAdminBarHeight()
							)
						);
...
twentytwenty.modalMenu = {
	...
	expandLevel: function () {
		...
		modalMenus.forEach( function ( modalMenu ) {
			var activeMenuItem =
				modalMenu.querySelector( '.current-menu-item' );

			if ( activeMenuItem ) {
				twentytwentyFindParents( activeMenuItem, 'li' ).forEach(
					function ( element ) {
						var subMenuToggle =
							element.querySelector( '.sub-menu-toggle' );
						if ( subMenuToggle ) {
							twentytwenty.toggles.performToggle(
								subMenuToggle,
								true
							);
						}
	...
	keepFocusInModal: function () {
		...
		_doc.addEventListener( 'keydown', function ( event ) {
			...
			if (
				clickedEl &&
				_doc.body.classList.contains( 'showing-modal' )
			) {
				...
				if ( '.menu-modal' === toggleTarget ) {
					...
					elements.unshift(
						_doc.querySelector( '.close-nav-toggle' )
					);
		...
		function removeFocusEsc( e ) {
			...
			if ( isEscape ) {
				var parentLi = focusedElement.closest( 'li' ),
					nestedParent = closestExcludingSelf(
						parentLi,
						'li.menu-item-has-children'
					),
					focusPosition = nestedParent
						? nestedParent.querySelector( 'a' )
						: false;
	...
	performToggle: function ( element, instantly ) {
		...
		setTimeout( function () {
			...
			if ( targetString === 'next' ) {
				...
				_doc.querySelector(
					'*[data-toggle-target="' + targetString + '"]'
