<?php

/** 
 * Template Name: Our Leadership Team
 * Template Post Type: post, page
 * @package WordPress
 * @subpackage YourThemeName
*/
?>

<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <style>

  .treasuries-hero-section-body { 
      height: 300px;
      position: relative;  
      text-align: center;
      align-content: center;
      background: url('<?php echo get_template_directory_uri(); ?>/assets/images/blue_backgorund.png') center/cover no-repeat;
      background-color: #000;
}


  .treasuries-hero-section-body h2 {
    font-family: 'Calibri', sans-serif;
    margin-bottom: 20px;
    font-weight:900;
    font-size: 50px;
    color: #fff;
  }

.treasuries-hero-section-body h4 {
  font-family: 'Calibri', sans-serif;
  margin-bottom: 10px;
  font-size: 15px;
  color: #fff;
  }

  </style>
</head>
<body>
  <div class="treasuries-hero-section-body">
  <h2>Trade Government Treasuries</h2>
        <h4>
          Treasuries, government bonds, and interest rate products offer exposure to fixed income markets. Bond prices move inversely to interest rates, providing portfolio diversification opportunities.
         </h4>
</div>
</body>
</html>
